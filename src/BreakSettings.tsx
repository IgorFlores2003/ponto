...
export default function BreakSettings({ request, onError }: { request: <T>(path: string, body?: unknown) => Promise<T>; onError: (error: unknown) => void }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try { await request(editing ? `/break-rules/${editing}` : '/break-rules', form); setForm({ name: '', starts_at: '', ends_at: '', effective_from: today() }); setEditing(null); await load(); setNotice(editing ? 'Pausa atualizada.' : 'Pausa cadastrada. Será identificada nas novas batidas a partir da vigência.') }
  ...
  return <section>{deleteRule && <ConfirmDialog title="Excluir pausa?" message={`Excluir ${deleteRule.name}? As batidas antigas continuarão preservadas.`} confirmLabel="Excluir pausa" danger busy={busy} onConfirm={() => void deactivate()} onCancel={() => setDeleteRule(null)} />}<h2>Pausas da equipe</h2><p className="break-explanation">Cadastre os horários das pausas para café. A faixa indica quando a batida pode iniciar essa pausa; o retorno acontece quando o funcionário bater o PIN novamente.</p>
