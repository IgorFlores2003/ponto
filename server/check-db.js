...
try {
  ...
  if (db.client.config.client === 'pg') {
    const { rows } = await db.raw("select count(*)::int as count from pg_tables where schemaname = 'public' and tablename in ('employees','entries','admins','sessions','settings','attempts') and rowsecurity")
    if (rows[0].count !== 6) throw new Error('RLS incompleto')
    console.log('RLS verificado nas seis tabelas do aplicativo.')
  }
