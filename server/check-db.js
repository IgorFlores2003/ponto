...
try {
  ...
  if (db.client.config.client === 'pg') {
    const { rows } = await db.raw("select count(*)::int as count from pg_tables where schemaname = 'public' and tablename in ('employees','entries','admins','sessions','settings','attempts','break_rules') and rowsecurity")
